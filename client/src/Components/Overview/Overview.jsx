...
class Overview extends React.Component {
  ...
  changePhoto(event) {
    ...
    if (event.target.id === "forward") {
      ...
      this.setState({
        prevPhotoUrl:
          this.state.styles[this.state.currentStyle].photos[currentPhotoId]
            .url || null,
      }); //prev photo is former current photo
      this.setState({
        currentPhotoUrl:
          this.state.styles[this.state.currentStyle].photos[currentPhotoId + 1]
            .url || null,
      }); //new current photo is old + 1
      this.setState({
        nextPhotoUrl:
          this.state.styles[this.state.currentStyle].photos[currentPhotoId + 2]
            .url || null,
      }); //new next photo is old + 2
      ...
      if (currentPhotoId === 1) {
        ...
        this.setState({
          currentPhotoUrl:
            this.state.styles[this.state.currentStyle].photos[0].url,
        }); //new current photo is the first photo
        this.setState({
          nextPhotoUrl:
            this.state.styles[this.state.currentStyle].photos[1].url,
        }); //new next photo is second photo
      } else {
        this.setState({
          prevPhotoId: currentPhotoId - 2 ? currentPhotoId - 2 : null,
        }); //prev photo is old - 2
        this.setState({
          currentPhotoId: currentPhotoId - 1 ? currentPhotoId - 1 : 0,
        }); //new current photo is old - 1
        this.setState({
          nextPhotoId: currentPhotoId - 1 ? currentPhotoId - 1 : null,
        }); //new next photo is old
        this.setState({
          prevPhotoUrl: this.state.styles[this.state.currentStyle].photos[
            currentPhotoId - 2
          ].url
        ...
        this.setState({
          currentPhotoUrl: this.state.styles[this.state.currentStyle].photos[
            currentPhotoId - 1
          ].url
            ? this.state.styles[this.state.currentStyle].photos[
                currentPhotoId - 1
      ...
      this.setState({
        nextPhotoUrl:
          this.state.styles[this.state.currentStyle].photos[currentPhotoId].url,
      }); //new next photo is old
    } else {
      this.setState({
        prevPhotoUrl:
          this.state.styles[this.state.currentStyle].photos[
            Number.parseInt(event.target.id) - 1
          ] || null,
      });
      this.setState({
        currentPhotoUrl:
          this.state.styles[this.state.currentStyle].photos[
            Number.parseInt(event.target.id)
          ].url,
      });
      this.setState({
        nextPhotoUrl:
      ...
      this.setState({
        prevPhotoId: Number.parseInt(event.target.id) + 1 || null,
      });
      this.setState({ currentPhotoId: Number.parseInt(event.target.id) });
      this.setState(
        { nextPhotoId: Number.parseInt(event.target.id) - 1 } || null
      );
    }
  }
  ...
  changeStyle(event) {
    ...
    this.setState({
      prevPhotoUrl: this.state.styles[id].photos[this.state.maxLength - 1].url,
    });
  ...
  componentDidMount() {
    ...
    axios.get(`/products/overview/${this.props.currentProduct}`).then((res) => {
      ...
      this.setState({
        prevPhotoUrl:
          styles.results[0].photos[styles.results[0].photos.length - 1].url,
        currentPhotoUrl: styles.results[0].photos[0].url,
        nextPhotoUrl: styles.results[0].photos[1].url,
        styles: styles.results,
        inventory: styles.results[0].skus,
        maxLength: styles.results.map((id) => id.photos).length,
        productInfo: product,
      });
    });
    // axios
  ...
  render() {
    ...
    if (this.state.modalOn) {
      modal = (
        <Modal
          trackClicks={this.props.trackClicks}
          productInfo={this.state.productInfo}
          photos={this.state.styles[this.state.currentStyle].photos}
          currentPhotoUrl={this.state.currentPhotoUrl}
      ...
      <div className="ov-main">
        ...
        <div>
          <Gallery
            ...
            prevPhotoUrl={this.state.prevPhotoUrl}
            prevPhotoId={this.state.prevPhotoId}
            nextPhotoUrl={this.state.nextPhotoUrl}
